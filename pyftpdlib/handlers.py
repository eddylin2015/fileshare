...
class FTPHandler(AsyncChat):
    ...
    def decode(self, bytes):
        return bytes.decode(localcode, self.unicode_errors)
    ...
    def push(self, s):
        asynchat.async_chat.push(self, s.encode(localcode))
                ...
                try:
                    ...
                    for x in listing:
                        if not isinstance(x, unicode):
                            x = unicode(x, localcode)
                        ls.append(x)
                    listing = sorted(ls)
                data = '\r\n'.join(listing) + '\r\n'
            data = data.encode(localcode, self.unicode_errors)
    ...
    def ftp_MLST(self, path):
        ...
        try:
            ...
            data = data.decode(localcode, self.unicode_errors)
