const {ReqAuth} = require('./coolauth_users');
...
function coolauth(req,res)
{
        ...
        if(!auth) {     // No Authorization header was passed in so it's the first time the browser hit us
                ...
                var username = creds[0];
                var password = creds[1];
                var result_=ReqAuth(username,password);
                //if((username == 't1') && (password == '***')) {   // Is the username/password correct?
                if(result_){
		      return result_ ;
                      //res.statusCode = 200;  // OK
                      //res.end('<html><body>Congratulations Login!</body></html>');
                }else {
