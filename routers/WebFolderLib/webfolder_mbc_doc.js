...
async function WebRouter(req, res, FileShare_path="", base_url="", adminRight=false) {
    ...
    let uploadDir=(FileShare_path+`\\www`).replace("/\\","/")
    let curr_path=uploadDir+subpath;
    console.log("*", method, url,subpath, curr_path);
    if (method=="GET"&&req.url.indexOf("ictdoc/build/html/")>-1&&req.url.match(/[.](js|htm|py|css|txt|dot|svg|jpg|png|ico|zip|csv)/g)) {
      let filepath = uploadDir + "/" + decodeURI(req.url.replace(base_url,""));
      return tools.down_pip_file(filepath.replace(/[/]+/g, "/").split("?")[0], res)
    }
    ...
    ///
    else if(req.url.indexOf('/picgrid')>-1 )
    {
      ...
      files.forEach(function (file) {
        ...
        if (file.toUpperCase().indexOf(".JPG")>-1||file.toUpperCase().indexOf(".PNG")>-1||file.toUpperCase().indexOf(".GIF")>-1||file.toUpperCase().indexOf(".WEBP")>-1) {
              if(subpath=="/") subpath=""
              res.write(`<div><img style="max-width: 100%;" src=${base_url}/${subpath}/down?file=` + encodeURI(file) + "><br>" + file + "</div>");//
          }
      });
    ...
    else if(req.url.indexOf('/pic')>-1 )
    {
      ...
      files.forEach(function (file) {
        ...
        if (file.toUpperCase().indexOf(".JPG")>-1||file.toUpperCase().indexOf(".PNG")>-1||file.toUpperCase().indexOf(".GIF")>-1||file.toUpperCase().indexOf(".WEBP")>-1) {
              if(subpath=="/") subpath=""
              res.write(`<img style="max-width: 120%;" src=${base_url}/${subpath}/down?file=` + encodeURI(file) + ">" + file + "<br>");//
          }
      });
      res.end();
      return;
        ...
        let file_stat=fs.statSync(curr_path +"/"+ file);
          if (file_stat.isDirectory()) {
              ...
              if(file.indexOf(".mp4")>-1||file.indexOf(".webm")>-1){
                let href_ios_video=`${base_url}/${encodeURI(subpath)}/playSafariVideo?file=` + encodeURI(file)
  ...
  else if(req.method == 'POST' && req.url.indexOf("/cmd")>-1)
    {
	  ...
	  let file_max_size=8000 * 1024 * 1024;
	  if(req.url.indexOf('fileSize=')>-1) file_max_size=Number(req.url.split("fileSize=")[1]);
     ...
     //let form = new formidable.IncomingForm(); // form mulit-par
     form.parse(req, (err, fields, files) => {
        if (err) {
          console.error(err);
          res.writeHead(err.httpCode || 400, { 'Content-Type': 'text/plain' });
        ...
        let file=Array.isArray(files.file1)? files.file1[0]: files.file1
        if(file.originalFilename){
          let file_name_=file.originalFilename.replace(/[\/]/g,"___")
          file_name_=file_name_.split("___")
          
          if(file_name_.length==1){
            if(file) fs.promises.rename(file.filepath, path.join(form.uploadDir, file.originalFilename));
          }else{
